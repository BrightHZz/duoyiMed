...
| Phase 6 | 论文撰写与投稿 | 学术写作 / Humanizer / Translator | 初稿 → 润色 → 投稿包 → 中文翻译 |
| Phase 7 | 临床工具部署 | 临床工具开发者 | Streamlit Web 工具、可执行文件打包 |

- **Phase 0-6** 由 `phase6_runner.py` 驱动完整流水线：预检 → 图表 → 表格 → 章节(LLM) → Humanize → 汇编 → Gate 6 → 英译中
...
- **跨项目学习**: 自动扫描历史项目的经验教训，在新项目 Gate 检查中阻断已知问题模式（如伪造文献、代码安全风险）
...
| `direction_discovery` | 综述驱动选题发现，挖掘高分综述缺口，自动过滤不可行方向 |
...
- **共享服务 (9)**: 数据工程师、生物统计学家、ML 工程师、学术写作、科研助理、Humanizer、临床工具开发者、图表设计师、医学翻译

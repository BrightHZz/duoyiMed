...
- **引用质量标准**: 所有引用必须符合 `company/reference/reference-quality-standard.md` 的六条规则 (一~五为共性, 六为临床常识豁免):
  - 规则一 (已发表): 只引用同行评议期刊论文或权威机构正式发表的指南/共识, 排除会议摘要、预印本、白皮书、新闻稿
  - 规则二 (质量优先): 高IF期刊优先, 近5年≥80% + 近10年≥95% (双重门槛), 一次文献优先, 综述不可作为论据引用, 经典文献 ≤5%
  - 规则三 (相关性): 每篇引用必须直接支撑论文论点, 不得"凑引用量", 单句引用上限 2 篇
  - 规则四 (避免教科书): 不引用教科书, 基础概念追溯到一次研究文献
  - 规则五 (来源层级已读验证): 全部引用 source_tier ≥ L2, L3 禁止。L1 = 全文已读, L2 = PubMed 摘要已验证。每篇标注 verified_date + pmid
  - 规则六 (临床常识豁免): 标准术式定义/解剖术语/分级系统名称无需引用
- **引用来源唯一性原则 (OEMC-R6)**: 所有参考文献必须来自两个已验证来源之一, 禁止从 LLM 记忆直接生成:
  - ✅ 来源A — `evidence-extraction-table.md`: Phase 3' 经 PubMed 验证录入的核心文献, 可直接引用
  - ✅ 来源B — `literature-matrix.md`: clinical-researcher 产出的背景文献矩阵, 可直接引用
  - ❌ 来源C — LLM 记忆: 严禁仅凭训练数据记忆写入引用。若写作过程中发现需要一篇不在 A/B 中的文献 → 提交 research-assistant 执行 PubMed 检索验证 → 补录入 literature-matrix → 方可引用
  - ❌ 严禁为满足引用数量门槛 (综述≥45篇) 而从 LLM 记忆批量生成参考文献
- 绝不编造数据、引用或结果
...
#### 铁律 4-1 — Results 结构：技术比较类综述按「平台」分组

**适用场景**: 多种技术共享同一核心平台（成像模态、设备类型）但使用不同解剖路径时。

**反模式**（按路径平铺）:
```
## CT-Guided Transgluteal Biopsy
## CT-Guided Transperineal Biopsy
```
→ 割裂了共享的 CT 工作流、辐射暴露、介入放射学归属

**正确模式**（按平台分组）:
```
## [平台名称]-Guided Biopsy
  总述段: 共享工作流、设备需求、辐射暴露、临床团队归属、两种解剖通道概述
  ### [解剖路径 A]
  ### [解剖路径 B]
```
...
| 场景 | 按平台 | 按路径 | 示例 |
|------|:--:|:--:|------|
| 多种成像模态, 同一穿刺路径 | ✅ | | 经会阴的 US / CT / MRI 引导 |
| 同一成像设备, 多种解剖路径 | ✅ | | CT 引导的经臀 / 经会阴 |
| 同一路径, 无设备差异 | | ✅ | 经会阴的 自由手 / 模板 / 融合 |
| 混合场景 | 以临床决策树为主线 | | — |

**效果**: 汇总表的行数 = Results 的 ## 节数 = Table 1 的分组数, 形成 1:1:1 三角映射。

#### 铁律 5 — Conclusion：连贯段落叙述，禁止参考文献引用

**犯的错**: (1) 把结论写成编号清单（1/2/3/4 或 1.1/1.2），破坏了学术论文行文的连贯性——结论是全文的收束，不是操作手册；(2) 在 Conclusion 中引用参考文献 [N]——引用证据是 Results/Discussion 的事，结论不应出现新的引用标记。

**正确结构**:
```
开头段：总结核心发现（2-3句，点明研究问题和主要结论）

中间段：按临床重要性递减展开论述（自然段落，非编号清单）
  - 各主题用段落过渡衔接，不用编号分隔
  - 每个主题的证据支撑应在 Results/Discussion 中已完成

末段：临床建议 + 展望（具体方向和条件，非空泛标语）
```

**禁止**:
- 编号清单格式（1/2/3/4 或 1.1/1.2）
- 参考文献引用 [N] — 所有引用证据在 Results/Discussion 中完成
- 新论点/新概念/新数据

**自检**: 遮掉段落首句通读一遍，能否感受论证的递进逻辑；全文搜索 `[` 确认 Conclusion 段无引用标记。
...
- 效应量必须与置信区间一起报告
- 统计显著性 ≠ 临床重要性
- **期刊文献 DOI 覆盖率 ≥ 95%** (允许增刊/在线补充材料等特殊情况无 DOI)。书籍和指南可用 URL 替代 DOI, 不计入期刊 DOI 计算。**全部文献 DOI/URL 覆盖率 ≥ 80%**
...
- [ ] **reference-claim-mapping.md 已生成**: 每篇 [N] → 正文声明一句话摘要, 含 `source_tier: L1/L2` + `verified_date: YYYY-MM-DD` + `pmid: XXXXXXXX`。运行 Gate 6 验证双向一致性 (孤儿引用=0, 幽灵声明=0)
- [ ] **来源层级达标**: 全部引用 source_tier ≥ L2, L3=0 (禁止), L2 ≤30% (COND_PASS)。L1 = 全文已读, L2 = PubMed 摘要已验证。层级标注写入 reference-claim-mapping.md
- [ ] **经典文献占比 ≤ 5%**: 在 classic-papers.md 注册或标注 [Classic] 的文献 ≤ 总参考文献数的 5%。超过上限 → 多余经典不再豁免，计入时效分母 → 可能导致 recency FAIL。写完全文后检查 classic_ratio
- [ ] **每篇参考文献必须在正文中被至少引用一次**: References 中的每个 [n] 必须在正文中出现。禁止为满足 recency 双重门槛 (近5年≥80% + 近10年≥95%) 而堆砌近期但无关的文献。写完全文后，交叉检查 References 列表与正文引用，确保 1:1 对应。
- [ ] **参考文献必须是一次文献**: 综述 (review) 和系统综述 (systematic review) 是二次文献，不可作为论据引用。所有论据必须追溯到原始研究。Meta-analysis 例外：可作为效应量综合估计引用，但需标注为 secondary source。原始研究占比 ≥ 80% (综述/Meta分析 ≤ 20%, 且仅限效应量综合估计或缺口识别背景引用)。
...
- [ ] **Methods↔Results 结局指标 1:1 验证**: Methods「Outcomes of Interest」声明的每个结局指标在 Results 中有对应的系统性定量回报（≥2处定量数据或表格行）。缺失回报的指标 → 补充 Results 段落或修改 Methods 声明为「可提取但报告不一致」

...
Step 2: 实时文献检索 (使用 WebSearch)
  ...
  检索 4 — 高分综述缺口挖掘 (Nature Reviews/Lancet/Annual Review):
  "[P 关键词] AND (review[Publication Type] OR systematic review)
   AND (future directions OR research gap OR unmet need OR open question)"
  示例: "frailty AND review AND future directions 2025 2026"

  目标期刊优先级:
  Tier 1: Nature Reviews Disease Primers, The Lancet, Lancet Healthy Longevity,
           Annual Review of Medicine, BMJ
  Tier 2: Ageing Research Reviews, GeroScience, JAGS review, European Urology reviews
  Tier 3: Frontiers in Medicine review, BMC series reviews

  提取内容:
  - 综述中明确标注的 "unresolved questions" / "future research directions"
  - 方法学共性批判 (共性缺陷 = 创新机会)
  - 被多篇综述同时提及的方向 (高共识 = 真实缺口)
  - 近2年新出现但尚未充分研究的方向

  注意: 综述本身不作为引用目标，仅作为找原始研究和缺口的入口
...
Step 4: 生成可行性判断
  - **能力匹配**: 对照 company/capability-profile.md 判断方向属于 Tier 1/2/3
    → Tier 3 (需湿实验/前瞻性入组/RCT) 直接建议放弃
...
### 综述缺口分析 (Review-Derived Gaps)
- 检索高分综述: [N] 篇
- Top 3 缺口方向:
  1. **[方向名称]** — 来源: [综述], 缺口类型: [数据/方法/人群/验证], 竞争度: [高/中/低]
  2. ...
  3. ...
- 与当前方向的交叉验证: [一致 → 最强可行性信号 / 部分一致 → 建议调整 / 不一致 → 审视方向合理性]
...
- **综述缺口优先** — 高分综述中明确标注的 "unresolved question" 若与研究方向一致，这是最强的可行性信号，应在报告中重点标注
- **多综述交叉验证** — 若 ≥2 篇高分综述共同指出同一缺口方向，标注为「高共识缺口」
- **参考文献时效性（双重门槛）** — 优先检索和纳入近 5 年文献；经典文献 (>5年) 仅限方法学奠基性论文。最终参考文献须满足: ① 近5年 ≥80% ② 近10年 ≥95%。经典论文 (已在 classic-papers.md 注册) 不计入时效分母
- **专科常识测试** — 在证据提取前，对每条候选引用的被引事实进行测试: *"该领域专科医生在不需要引用的情况下已经知道这个事实吗？"* ⬇ 通过测试的条目从参考文献中移除，在正文中作为无引用的常识陈述:
  - ✅ 标准术式定义（如 APR、TME、经会阴穿刺）— 已在教科书中成为基础知识 → **不引用**
  - ✅ 解剖学术语（如"会阴皮肤""盆底""直肠系膜""ischiorectal fossa"）→ **不引用**
  - ✅ 广泛使用的分级/分期系统名称（如 Gleason 分级、Clavien-Dindo 分级、PI-RADS）→ **不引用**
  - ❌ 具体数值声明（如"检出率 82.1%""脓毒症率 0.5-3.0%"）→ **必须引用**
  - ❌ 指南推荐意见（如"EAU 倾向经会阴为标准方案"）→ **必须引用**
  - 效果：减少为常识添加的无意义旧引用，直接降低经典文献占比，提升时效性
- **经典文献标注** — 引用超过 5 年的文献时（且未通过专科常识测试），必须在 References 中标注豁免理由：
...
### 6. 综述驱动方向发现 (Review-Driven Direction Discovery) — ⭐ 新增

**触发条件**: 收到 `direction_discovery` 任务或用户请求包含 "找方向" / "挖掘选题" / "有什么可做的" 等关键词时。

**与 §5 选题预检的区别**:
- **选题预检 (§5)**: 用户**有方向** → 验证可行性 (能不能做?)
- **方向发现 (§6)**: 用户**无方向** → 系统挖掘候选方向 (有什么值得做?)

#### 6.1 执行策略

```
检索 1 — 领域总览综述:
  "[领域关键词] AND (review[Publication Type]) AND (state of the art OR comprehensive review) 2024 2025 2026"
  → 目标: Nature Reviews, Annual Review, The Lancet 系列

检索 2 — 缺口专项:
  "[领域关键词] AND (future directions OR research gap OR unmet need OR open question) 2024 2025 2026"
  → 目标: 综述 Discussion 部分的 "unresolved questions"

检索 3 — 方法学批判:
  "[领域关键词] AND (methodological challenges OR limitations AND review) 2024 2025 2026"
  → 目标: 找方法共性缺陷 = 创新机会

检索 4 — 新兴方向:
  "[领域关键词] AND (emerging OR novel approach OR new paradigm) AND review 2024 2025 2026"
  → 目标: 近2年新出现的方向
```
...
- **必须执行 WebSearch** — 不允许仅凭 LLM 记忆回答
- **能力边界硬约束** — 候选方向必须符合公司能力画像 (见 company/capability-profile.md)。Tier 3 (需要湿实验/前瞻性入组/RCT) 的方向直接排除，不可出现在候选列表中
- **Tier 2 标注** — 需要临床合作方数据的方向必须标注「需 [合作类型] 合作」，并说明合作方可替代性与交付周期预估
- **不考虑数据可用性** — 数据可行性由后续的 PI FRAME 评估负责
- **产出候选方向，不做最终决定** — 最终的选题决策由 PI 在 FRAME 评估中完成
- **多综述共识标注** — 若 ≥2 篇高分综述共同指出同一缺口方向，标注为「高共识缺口」
- **诚实报告竞争度** — 如果某方向已有大量团队在做，如实标注竞争度为「高」
...
### 7. 自主知识库监测 (Autonomous KB Monitoring) — ⭐ 新增
...
### 8. 综述选题方向发现 (Review Topic Discovery) — ⭐ 新增

**触发条件**: 收到 `direction_discovery` 任务且用户请求包含 "综述选题" / "综述方向" / "写什么综述" 等关键词时。

**与 §5、§6 的区别**:
- **选题预检 (§5)**: 有具体论著方向 → 验证可行性
- **论著方向发现 (§6)**: 无论著方向 → 从综述中挖掘研究缺口 → FRAME 评估
- **综述选题发现 (§8)**: 无论著方向 → 评估现有综述覆盖与时效性 → RGA 评估

#### 8.1 核心逻辑

论著选题问"有什么研究没做过？" 综述选题问"有什么综述没写过 / 该更新了？"

RGA 评估框架:
- **R (Recency — 时效性)**: 最近一篇高质量系统综述是何时? ≥3年=高潜力
- **G (Gap Analysis — 覆盖缺口)**: 现有综述未覆盖什么?
- **A (Available Evidence — 证据量)**: 新增原始研究量是否足够支撑一篇综述? ≥15-20篇=适合
- **S (Scope & Feasibility — 范围与可行性)**: PICO 可清晰定义? 范围可控?

#### 8.2 检索策略

```
检索 1 — 现有综述扫描: 找出已有综述, 评估覆盖和时效
检索 2 — 综述缺口识别: 已有综述自身的局限性声明
检索 3 — 新增证据量评估: 自上次综述后的新原始研究数量
检索 4 — 无综述蓝海方向: 有大量原始研究但无系统综述的方向
```

#### 8.3 综述选题优先级

1. **尚无系统综述** (蓝海, 撰写首篇 PRISMA 系统综述) → 最高优先级
2. **有重大更新需求** (末次综述 ≥3 年 + ≥15-20 篇新原始研究) → 高优先级
3. **方法学升级** (叙述性综述 → 系统综述; 不做 Meta-analysis → 做 Meta-analysis) → 中优先级
4. **已有最新 Cochrane/高分系统综述且覆盖完整** → 不建议选题

#### 8.4 强制约束

- **必须执行 WebSearch** — 不允许仅凭 LLM 记忆判断"是否已有系统综述"
- **全部 Tier 1** — 系统综述/Meta-analysis = 纯计算, 不需要能力过滤
- **纳入文献量预估** — 必须估算可纳入的原始研究数量, 不可凭空承诺 ≥45 篇
- **诚实报告已有综述** — 若已有高质量 Cochrane 综述且覆盖全面, 直接建议放弃

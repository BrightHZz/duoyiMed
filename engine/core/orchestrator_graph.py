...
返回 JSON:
{
  "intent": "new_project | literature_review | paper_writing | quick_consult | status_check | kb_enrich | direction_discovery",
...
- direction_discovery: 用户想找研究方向, 没有具体假说, 希望从文献中挖掘选题方向。
  关键词: "找方向"、"有什么可做的"、"下一个课题"、"选题方向"、"research direction"、"topic idea"
"""
...
KB_ENRICH_PHASES = {
    "kb_search": {
        "agents": ["research-assistant"],
        "parallel": False,
        "description": "知识库检索: 按预配置检索式搜索最新文献",
        "next": "kb_ingest",
        "expected_outputs": [
            "kb_enrich_search_results.json",
        ],
    },
    "kb_ingest": {
        "agents": ["research-assistant"],
        "parallel": False,
        "description": "知识库入库: 验证DOI → 查重 → 写结构化笔记",
        "depends_on": ["kb_search"],
        "next": None,
        "expected_outputs": [
            "kb_enrich_ingest_report.md",
        ],
    },
}
    ...
    "writing": {
        "execution": {
            "signals": [
                "[数据待确认]", "[数据矛盾]", "[数值不一致]",
                "[结果缺失]", "data missing",
                # 🆕 队列筛选数据缺失/不一致 (2026-05-14)
                "cohort_attrition.json 缺失", "cohort_attrition.json not found",
                "Figure data.json 缺失", "figure_data.json missing",
                "筛选 N 与 cohort_attrition.json 不一致",
                "Figure 数据与 cv_results.json 不一致",
            ],
            "severity": "critical",
            "action": "reopen_gate",
            "message": "写作阶段发现数据缺失或矛盾, 需返回上游确认。若为 cohort_attrition.json 缺失 → 触发 Phase 3 重开; 若为 Figure data.json 缺失或数值不一致 → 触发 Phase 6 figures 返工。",
        },
        "problem_definition": {
            "signals": [
                "缺少关键的临床背景", "研究意义不明确",
...
class ResearchOrchestrator:
    ...
    def run(self, user_request: str, project_dir: str = None) -> str:
        ...
        elif intent["intent"] == "kb_enrich":
            result = self._run_kb_enrich_workflow(user_request, intent)
        elif intent["intent"] == "status_check":
            result = self._run_status_check(intent)
        elif intent["intent"] == "direction_discovery":
            result = self._run_direction_discovery(user_request, intent, project_dir)
        else:
            result = self._run_quick_consult(user_request, intent)
    ...
    def _fallback_classify(self, user_request: str) -> dict:
        ...
        # KB 富化检查
        if any(w in req_lower for w in ["充实知识库", "知识库富化", "自主检索", "kb_enrich", "文献监测"]):
            intent = "kb_enrich"
        # 方向发现检查
        elif any(w in req_lower for w in ["找方向", "新方向", "有什么可做", "下一个课题",
                "选题方向", "research direction", "topic idea", "挖掘选题", "找课题",
                "值得做的方向", "研究方向推荐"]):
            intent = "direction_discovery"
        # 文献综述优先检查 (避免被"预测"/"模型"等关键词先匹配到 new_project)
        elif any(w in req_lower for w in ["文献", "综述", "检索", "prisma", "扫描", "搜索", "阅读笔记"]):
...
现在开始设计。"""
        ...
        for step in steps:
            ...
            if step.get("parallel") and len(resolved_agents) > 1:
                for agent_id in resolved_agents:
                    actual_input = task_input
                    # 🆕 figures 步骤注入完整数据上下文 (防止 LLM 推测 Figure 数字)
                    if step_name == "figures" and agent_id.endswith("ml-engineer"):
                        data_context = self._build_agent_input(
                            agent_id, f"{phase_id}_{step_name}",
                            user_request, previous_outputs, step_results, project_id,
                        )
                        actual_input = data_context + "\n\n---\n\n" + task_input
                    result = self._call_agent(
                        agent_id, actual_input,
    ...
    def _summarize_step_results(step_results: dict) -> str:
        ...
        ⚠️ 使用 **bold** 而非 ### 子标题, 防止 LLM 将摘要结构镜像到论文正文中
        (如 Methods/Results 的 ### 子标题不应受前序步骤输出结构的影响)。
        """
        if not step_results:
            return "(无前序步骤产出)"
        parts = []
    ...
    def _write_phase6_files(self, step_results: dict, project_id: str) -> dict:
        """将 Phase 6 多步骤产出的内存字符串写入磁盘 (仅零件层)。

        零件层: sections/ tables/ figures/ — Agent 产出的分章节/表格/图表零件
        投稿层: 由 run_assembly.py 脚本负责组装，不在此方法中创建
        ...
        # ---- 定位各步骤输出 ----
        # 注: assembly 不再从 LLM 文本解析，改由 run_assembly.py 脚本执行
        sections_text = ""
        tables_text = ""
        figures_text = ""
        humanize_text = ""
        ...
        # ---- 转换 tables .md → .csv → tables/ (零件层, 供 run_assembly.py 复制) ----
        if table_md_files:
            try:
                tables_dir = proj_dir / 'tables'
                tables_dir.mkdir(parents=True, exist_ok=True)
                ...
                for tname, md_content in table_md_files.items():
                    ...
                    if csv_content:
                        ...
                        (tables_dir / csv_name).write_text(csv_content, encoding='utf-8')
                        written.append(f'tables/{csv_name}')
                        csv_count += 1
                if csv_count:
                    print(f"  ✅ tables/ CSV 转换完成 ({csv_count} 文件)")
            except Exception as e:
                print(f"  ⚠️ tables CSV 转换失败 ({e})")
        ...
        # 投稿层 (submission/) 由 run_assembly.py 脚本负责组装，不在此处手动创建

        if written:
            print(f"  📁 零件层共写入 {len(written)} 个文件 → {proj_dir}")
    ...
    @staticmethod
    def _run_script_subprocess(script_path: str, project_dir: Path,
        ...
        try:
            ...
            return {"success": False, "exit_code": -1,
                    "stdout": "", "stderr": f"python not found on PATH"}
        except Exception as e:
            return {"success": False, "exit_code": -1,
                    "stdout": "", "stderr": str(e)}
...
## ⚠️ Round 1 任务: 选题文献 + 期刊趋势预检 + 综述缺口挖掘
...
3. **高分综述缺口挖掘**: 检索高分综述 (Nature Reviews/Lancet 系列/Annual Review 等),
   提取综述中标注的 "unresolved questions" 和 "future directions"
   → 补充到《选题文献预检报告》的「综述缺口分析」部分

以上报告将作为 PI 的 FRAME 评估中 F 维度和 M 维度的定量数据源。"""
...
→ 输出《建模可行性预检报告》(作为 FRAME 技术可行性参考)"""
    ...
    def _build_frame_quantitative_prompt(self, round1_outputs: dict, sds_text: str = "") -> str:
        ...
        lines += [
            ...
            "- 引用《文献预检报告》中「综述缺口分析」的缺口方向与共识度",
            "- 对标论文的样本量、数据源、方法对比",
            "- 基于实时文献数据 (非记忆) 判断本研究的竞争位置",
            "",
            "### R — Resource Audit (资源审计)",
            "- ⚠️ 对照 company/capability-profile.md, 判断研究方向属于 Tier 1/2/3",
            "- Tier 3: 直接建议放弃 (公司无此能力: 无湿实验/无前瞻性入组/无RCT 能力)",
            "- Tier 2: 明确需要什么合作方、合作方可替代性、合作方交付周期",
            "- Tier 1: 确认数据源在公司已有公共数据集中",
...
5. **输出格式**:
   ...
   - ## 确信度说明 (逐条标注)"""
    ...
    def _check_gate(self, phase_id: str, outputs: dict, project_id: str,
        ...
        skip_llm_review: 跳过 PI LLM 审查 (用于 kb_enrich 等轻量工作流)
        """
        ...
        # LLM 审查 (调用 PI, 轻量工作流可跳过)
        llm_checks = gate_def.get("llm_checks", [])
        if llm_checks and not skip_llm_review:
        ...
        elif llm_checks and skip_llm_review:
            # 轻量工作流: LLM 检查标记为 skip (auto checks 已足够)
            for i, check in enumerate(llm_checks):
                checks.append({
                    "check_id": f"llm_review_{i+1}",
                    "description": check,
                    "check_type": "llm",
                    "result": "skip",
                    "detail": "轻量工作流跳过 PI 审查",
                })
        ...
        skip_info = f", {skip_count}⊘" if skip_count > 0 else ""
        print(f"  [Gate {phase_id}] {status} ({pass_count}✓, {fail_count}✗{skip_info})")
        # 回填 Gate 状态到运行日志
...
## ⚠️ 模型评估完整性要求 (Gate 3 强制)

train_model.py 必须对所有模型（主模型 + baseline + 所有对比模型）输出统一的评估指标集。
每一个模型必须通过 `evaluate_model()` 函数返回以下全部指标:

```
区分度: AUC (ROC) + 95% CI, PR-AUC
校准度: Brier Score, Calibration Slope, Calibration Intercept
阈值性能: Sensitivity, Specificity, PPV, NPV, F1 Score
训练稳定性: CV fold 间 AUC 标准差
```

cv_results.json 结构必须为:
```json
{{
  "models": {{
    "xgboost": {{"auc": {{"mean": 0.842, "ci_low": ..., "ci_high": ...}}, "pr_auc": ..., "brier": ..., ...}},
    "logistic_regression": {{"auc": ..., "pr_auc": ..., "brier": ..., ...}},
    ...
  }}
}}
```

⚠️ 任一模型缺少任一必备指标 → Gate 3 FAIL → 必须返工补充。
不要只为最佳模型算全量指标, baseline 模型也必须覆盖。
""")
        ...
        if agent_id == "scientific-writer":
            parts.append(
                ...
                "⚠️ 上游文档结构隔离: 上游输出(如 SAP、临床审查)中的 ### 子标题是内部工作文档结构, "
                "不代表论文 IMRAD 章节的组织方式。Methods 仅限 5 个标准子标题, "
                "Results 仅限 5 个标准子标题。不要将上游文档的 ### 标题镜像到论文章节中。"
            )
        else:
    ...
    def run_kb_enrich(self, divisions: list[str] = None) -> str:
        """公开入口: 知识库自主富化 (由 CLI 直接调用, 无需 LLM 意图分类)

        Args:
            divisions: 限定的事业部列表, None = 全部事业部
        """
        if divisions is None:
            divisions = self.config.active_divisions

        all_reports = []
        for div in divisions:
            self.active_division = div
            print(f"\n[KB Enrich] 事业部: {div}")
            report = self._run_kb_enrich_workflow(
                user_request=f"自主检索最新文献, 充实 {div} 知识库",
                intent={"intent": "kb_enrich", "project_id": f"kb_enrich_{div}",
                        "summary": f"自主检索充实 {div} 知识库", "keywords": ["kb_enrich", div]},
            )
            all_reports.append(report)

        return "\n\n".join(all_reports)

    def _run_kb_enrich_workflow(self, user_request: str, intent: dict) -> str:
        """知识库自主富化工作流:
        Phase kb_search → Gate → Phase kb_ingest → Gate
        """
        import yaml as _yaml

        project_id = intent.get("project_id", f"kb_enrich_{self.active_division}")

        # 加载检索式配置
        searches_config_path = self.config.company_dir / "reference" / "saved-searches.yaml"
        if not searches_config_path.exists():
            return f"❌ 检索式配置文件未找到: {searches_config_path}"

        searches_config = _yaml.safe_load(searches_config_path.read_text(encoding="utf-8"))
        div_config = searches_config.get("divisions", {}).get(self.active_division, {})
        settings = searches_config.get("settings", {})

        if not div_config.get("topics"):
            return f"❌ 事业部 {self.active_division} 未配置检索式, 请在 {searches_config_path} 中添加"

        # 构建 enriched task input (包含检索式配置 + settings)
        enriched_request = f"""{user_request}

## 检索式配置 (saved-searches.yaml)

```yaml
{_yaml.dump({"division": self.active_division, "topics": div_config["topics"], "settings": settings}, allow_unicode=True)}
```

## 执行说明
1. 先执行 kb_search: 逐 topic 逐 query 调用 WebSearch, 输出候选论文列表到 kb_enrich_search_results.json
2. 再执行 kb_ingest: 验证 DOI → 查重 → 写文献笔记, 输出入库报告到 kb_enrich_ingest_report.md
3. 严格遵守 settings 中的 recency_days / min_relevance_score / max_ingest_per_run 约束"""

        # Phase 1: kb_search
        print(f"\n[Phase kb_search] 执行 {len(div_config['topics'])} 个 topic...")
        task_input = self._build_agent_input(
            agent_id="shared/research-assistant",
            phase_id="kb_search",
        ...
        search_result = self._call_agent("shared/research-assistant", task_input)
        search_outputs = {"shared/research-assistant": search_result}

        # Gate kb_search (仅 auto checks, 不调 PI)
        print(f"\n[Gate kb_search]")
        # 对于 kb_enrich, 我们不逐 phase 做 gate (免得两次调 PI), 而是在最后做整体 gate
        # 但 auto checks 需要 outputs 中包含搜索结果
        # 这里我们不在中间阶段做 gate, 统一在 kb_ingest 后做

        # Phase 2: kb_ingest
        print(f"\n[Phase kb_ingest] 验证 DOI → 查重 → 入库...")
        task_input = self._build_agent_input(
            agent_id="shared/research-assistant",
            phase_id="kb_ingest",
            user_request=f"{enriched_request}\n\n## Phase 1 搜索结果\n{search_result[:8000]}",
            previous_phase_outputs={"shared/research-assistant": search_result},
            current_phase_outputs={},
            project_id=project_id,
        )
        ingest_result = self._call_agent("shared/research-assistant", task_input)
        ingest_outputs = {"shared/research-assistant": ingest_result}

        # Gate kb_enrich (整体 Gate, 跳过 PI 审查)
        print(f"\n[Gate kb_enrich]")
        gate_result = self._check_gate("kb_enrich", ingest_outputs, project_id,
                                       skip_llm_review=True)

        # 格式化响应
        all_results = [
            f"## KB 富化报告 — {self.active_division} 事业部\n",
            f"**日期**: {datetime.now().strftime('%Y-%m-%d')}",
            f"**Gate 状态**: {gate_result['status'].upper()}",
            f"",
        ]
        if search_result:
            all_results.append(f"### Phase 1: 检索结果\n{search_result[:3000]}")
        if ingest_result:
            all_results.append(f"### Phase 2: 入库结果\n{ingest_result[:3000]}")

        all_results.append(f"\n### Gate 检查 ({gate_result['status']})")
        for c in gate_result.get("checks", []):
            icon = "✓" if c["result"] == "pass" else "✗"
            all_results.append(f"- {icon} **{c['check_id']}**: {c['detail']}")
...
请开始规划。"""
    ...
    def _run_direction_discovery(self, user_request: str, intent: dict,
                                  project_dir: str = "") -> str:
        """
        选题方向发现工作流。支持两种模式:
        - 论著选题: 从高分综述中挖掘研究缺口 → FRAME 评估
        - 综述选题: 评估现有综述覆盖与时效性 → RGA 评估
        """
        division = self.active_division
        is_review = self._is_review_discovery(user_request)
        mode_label = "综述选题 (RGA)" if is_review else "论著选题 (FRAME)"
        print(f"  [Direction Discovery] 事业部: {division} | 模式: {mode_label}")

        # Round 1: 综述/缺口挖掘
        if is_review:
            ra_input = self._build_review_discovery_input(user_request, division)
        else:
            ra_input = self._build_direction_discovery_input(user_request, division)
        ra_result = self._call_agent("shared/research-assistant", ra_input,
                                      phase_id="direction_discovery_r1", project_id="")
        print(f"  [Direction Discovery] Round 1 完成: {'综述选题挖掘' if is_review else '研究缺口挖掘'}")

        # Round 2: PI 排序推荐
        if is_review:
            pi_input = self._build_review_ranking_input(user_request, ra_result, division)
        else:
            pi_input = self._build_direction_ranking_input(user_request, ra_result, division)
        pi_result = self._call_agent(f"{division}/pi", pi_input,
                                      phase_id="direction_discovery_r2", project_id="")
        print(f"  [Direction Discovery] Round 2 完成: PI {'RGA' if is_review else 'FRAME'} 评估")

        # 聚合输出
        title = "综述选题方向报告" if is_review else "研究选题方向报告"
        result = (
            f"## {title}\n\n"
            f"### Round 1: {'综述选题挖掘' if is_review else '研究缺口挖掘'}\n\n{ra_result}\n\n"
            f"---\n\n"
            f"### Round 2: PI {'RGA' if is_review else 'FRAME'} 排序推荐\n\n{pi_result}\n\n"
            f"---\n\n"
            f"*事业部: {division} | 选题模式: {mode_label} | 如需启动项目，请使用 new_project 工作流*"
        )
        return result

    @staticmethod
    def _is_review_discovery(user_request: str) -> bool:
        """检测用户是否在寻找综述选题（而非论著选题）

        判定逻辑: 同时出现「综述类」关键词 + 「选题/方向」关键词 = 综述选题模式
        仅有综述关键词但无选题意图（如"帮我写一篇综述"）→ 不触发综述选题
        """
        req_lower = user_request.lower()
        has_review_term = any(kw in req_lower for kw in [
            "综述", "review", "系统评价", "meta分析", "范围综述", "meta-analysis"
        ])
        has_topic_term = any(kw in req_lower for kw in [
            "选题", "方向", "题目", "写什么", "写篇", "可以写", "topic",
            "有什么", "推荐", "做什么", "找什么"
        ])
        return has_review_term and has_topic_term

    def _build_direction_discovery_input(self, user_request: str, division: str) -> str:
        """构建 direction_discovery Round 1 中 research-assistant 的任务输入"""
        domain_keywords = {
            "geriatrics": "frailty OR sarcopenia OR cognitive decline OR multimorbidity OR aging",
            "urology": "urolithiasis OR prostate cancer OR bladder cancer OR BPH OR urinary tract infection",
            "pediatrics": "pediatric critical care OR neonatal OR child health OR pediatric emergency OR PICU",
        }
        domain_label = {
            "geriatrics": "老年医学",
            "urology": "泌尿外科",
        ...
        domain = domain_keywords.get(division, domain_keywords["geriatrics"])
        label = domain_label.get(division, "老年医学")

        return f"""## 综述驱动选题方向发现 — {label}

你的任务: 系统检索 {label} 领域的高分综述, 提取可操作的选题方向。

用户请求: {user_request}

## ⚠️ 公司能力边界 (硬约束)

duoyiMed 是纯计算团队, 没有实验室。推荐方向必须符合公司能力画像 (参见 company/capability-profile.md):

✅ 可以做 (Tier 1 — 完全自给):
- 公共数据集 (CHARLS/MIMIC/SEER/NHANES/GEO/TCGA 等) 的二次分析
- ML/DL 预测建模、因果推断、生信重分析
- 系统综述/Meta-analysis
- 回顾性数据分析 (需注明需要合作方提供数据 — 属于 Tier 2)
...
对于每个候选方向, 在「预估难度」之前增加一行:
- **能力匹配**: [Tier 1 — 完全自给 / Tier 2 — 需XX合作]
→ Tier 3 的方向直接排除, 不要出现在候选列表中

### 检索策略 (多轮)

检索 1 — 领域总览综述:
"{domain} AND (review[Publication Type]) AND (state of the art OR comprehensive review) 2024 2025 2026"
→ 目标: Nature Reviews, Annual Review, The Lancet 系列

检索 2 — 缺口专项:
"{domain} AND (future directions OR research gap OR unmet need OR open question) 2024 2025 2026"
→ 目标: 综述 Discussion 部分的 "unresolved questions"

检索 3 — 方法学批判:
"{domain} AND (methodological challenges OR limitations AND review) 2024 2025 2026"
→ 目标: 找方法共性缺陷 = 创新机会

检索 4 — 新兴方向:
"{domain} AND (emerging OR novel approach OR new paradigm) AND review 2024 2025 2026"
→ 目标: 近2年新出现的方向
...
⚠️ 强制约束:
- 必须执行 WebSearch, 不允许仅凭 LLM 记忆
- 不考虑数据可用性 (由 PI 后续评估)
- 产出候选方向, 不做最终决定"""

    def _build_direction_ranking_input(self, user_request: str, ra_result: str,
                                        division: str) -> str:
        """构建 direction_discovery Round 2 中 PI 的任务输入"""
        domain_label = {
            "geriatrics": "老年医学",
            "urology": "泌尿外科",
            "pediatrics": "儿科",
        ...
        return f"""## 候选选题方向评估 — {label}

research-assistant 已完成综述挖掘, 产出候选方向。以下是全文:

{ra_result[:5000]}

你的任务: 对每个候选方向做快速评估, 给出排序推荐。

用户原始请求: {user_request}

### 快速评估标准 (简版 FRAME)
- F (Field Scan): 该方向的竞争强度? 已有多少团队在做?
- R (Resource Audit): 对照 company/capability-profile.md, 能力匹配度? Tier 1=完全自给 / Tier 2=需合作方? Tier 3 已在 Round 1 排除, 此处只需确认 Tier 2 合作可行性.
- A (Alignment): 是否对齐基金/临床需求?
- M (Market Gap): 发表窗口如何?
- E (Edge): 我们有什么独特优势?
...
⚠️ 注意: 这是快速评估, 不是正式 FRAME 评估。最终选题决定在用户选择方向后进行正式 Phase 1 FRAME 评估。"""

    def _build_review_discovery_input(self, user_request: str, division: str) -> str:
        """构建综述选题方向发现的 research-assistant 任务输入 (RGA 框架)"""
        domain_keywords = {
            "geriatrics": "frailty OR sarcopenia OR cognitive decline OR multimorbidity OR aging",
            "urology": "urolithiasis OR prostate cancer OR bladder cancer OR BPH OR urinary tract infection",
            "pediatrics": "pediatric critical care OR neonatal OR child health OR pediatric emergency OR PICU",
        ...
        return f"""## 综述选题方向发现 — {label}

你的任务: 系统检索 {label} 领域, 发现值得撰写的系统综述/Meta-analysis 选题方向。

用户请求: {user_request}

## ⚠️ 综述选题标准 (RGA 框架)

综述选题不同于论著选题。你需要判断"有没有值得写的综述"，而非"有没有值得做的研究"。

评估维度:
- **R (Recency — 时效性)**: 最近一篇高质量系统综述是什么时候? ≥3年=高分; 只有叙述性综述=中分
- **G (Gap Analysis — 覆盖缺口)**: 现有综述未覆盖什么? 亚组/新方法/特定人群/新结局?
- **A (Available Evidence — 证据量)**: 自上次综述以来新增了多少可纳入的原始研究? ≥15-20篇=适合启动
- **S (Scope & Feasibility — 范围与可行性)**: PICO 是否可清晰定义? 范围是否可控?

### 检索策略

检索 1 — 现有综述扫描:
"{domain} AND (systematic review OR meta-analysis) 2020 2021 2022 2023 2024 2025 2026"
→ 目标: 找出已有综述, 评估覆盖范围和时效性

检索 2 — 综述缺口识别:
"{domain} AND (systematic review) AND (limitations OR future research OR updated OR research agenda) 2023 2024 2025 2026"
→ 目标: 找出已有综述自身声明的局限性

检索 3 — 新增证据量评估:
"{domain} AND (clinical trial OR cohort OR case-control OR prediction model OR machine learning) 2023 2024 2025 2026"
→ 目标: 统计末次综述发表后新增的原始研究数量

检索 4 — 无综述的蓝海方向:
"{domain} AND (randomized controlled trial OR prospective cohort) NOT (systematic review OR meta-analysis OR review)"
→ 目标: 找出有大量原始研究但尚无系统综述的方向
...
#### 方向 N: [综述题目]
- **综述类型**: [系统综述 / Meta-analysis / 范围综述 / 更新综述]
- **现有综述状况**: [无系统综述 / 最近1篇系统综述在20XX年 / 多篇叙事综述但无系统综述 / 有Cochrane综述但未更新]
- **新增证据量**: 自上次综述后约 [N] 篇新原始研究
- **覆盖缺口**: [现有综述未覆盖XX亚组/XX方法/XX结局]
- **RGA 评估**:
  - R (时效性): [高/中/低] — 说明
  - G (缺口): [高/中/低] — 说明
  - A (证据量): [高/中/低] — 约 N 篇新研究
  - S (范围): [高/中/低] — PICO [简述]
- **预估纳入文献数**: ≥[N] 篇
- **能力匹配**: Tier 1 — 完全自给 (系统综述 = 纯计算)
- **预估周期**: [2-4 个月]
- **目标期刊**: [Tier 1/2/3，具体期刊名]

... (共 3-5 个方向)

### 方向交叉矩阵
| 方向 | 时效性(R) | 缺口(G) | 证据量(A) | 范围(S) | 综合推荐 |
|------|-----------|---------|-----------|---------|---------|

⚠️ 强制约束:
- 必须执行 WebSearch, 不允许仅凭 LLM 记忆
- 综述选题全部属于 Tier 1 (纯计算), 不需要能力过滤
- 优先推荐: 尚无系统综述的蓝海方向 > 有重大更新需求的系统综述 > 方法学升级综述"""

    def _build_review_ranking_input(self, user_request: str, ra_result: str,
                                     division: str) -> str:
        """构建综述选题 PI 排序推荐的任务输入 (RGA 框架, 替代 FRAME)"""
        domain_label = {
            "geriatrics": "老年医学",
            "urology": "泌尿外科",
            "pediatrics": "儿科",
        ...
        return f"""## 综述选题方向评估 — {label}

research-assistant 已完成综述选题挖掘, 产出候选综述方向。以下是全文:

{ra_result[:5000]}

你的任务: 对每个候选综述方向做 RGA 快速评估, 给出排序推荐。

用户原始请求: {user_request}

### 快速评估标准 — RGA 框架 (综述专用, 非 FRAME)

**R — Recency (时效性)**
- 最近一篇高质量系统综述何时发表?
- 若 ≥3 年且新增原始研究 ≥15 篇 → 强烈推荐更新
- 若从无系统综述 → 最高优先级 (撰写首篇)

**G — Gap Analysis (覆盖缺口)**
- 现有综述是否覆盖了我们领域的关键亚组/方法/结局?
- 是否有方法学升级空间 (如从不做 Meta-analysis → 做 Meta-analysis)?
- 叙述性综述 → 系统综述 = 天然升级

**A — Available Evidence (证据量)**
- 是否有足够的新原始研究支撑一篇有意义的综述?
- ≥20 篇可纳入研究 → 高
- 10-19 篇 → 中 (可做范围综述)
- <10 篇 → 低 (建议等待更多证据积累)

**S — Scope & Feasibility (范围与可行性)**
- PICO 是否可清晰定义?
- 范围是否合适 (不空泛不狭窄)?
- 预估工作量: 2-4 个月 = 合理; >6 个月 = 范围可能过大
- 全部属于 Tier 1 (纯计算, 不需实验室/患者入组)
...
#### 🥇 第一推荐: [综述题目]
- **综述类型**: [系统综述/Meta-analysis/范围综述/更新综述]
- **RGA 简评**: R=[高/中/低] G=[高/中/低] A=[高/中/低] S=[高/中/低]
- **一句话推荐理由**: ...
- **目标期刊建议**: ...
- **下一步建议**: 立即启动 PRISMA 流程, 第一步是...
...
⚠️ 注意: 这是综述选题的快速评估。最终选题决定后, 应使用 literature_review 工作流启动正式的 PRISMA 2020 系统综述流程。"""

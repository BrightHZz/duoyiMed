...
| **Phase 7** (临床部署) | shared/clinical-tool-developer | `company/shared-services/clinical-tool-developer-agent.md`; 详见下方 Phase 7 章节 |
...
- **去AI味**: 规则库见 `company/reference/humanizer-rules.md` (36项禁用词检测); 执行流程见 `company/shared-services/humanizer-agent.md`
- **缩写规范**: 所有缩写首次出现必须给出全称 (通用缩写 DNA/RNA/BMI/CI/AUC/OR/HR/SD 除外)
- **Discussion 七段式 & IMRAD 结构**: 见 `company/shared-services/scientific-writer-agent.md`
- **数值精度规范**: 见 `company/shared-services/scientific-writer-agent.md`
- **模型评估指标 & ML 安全**: 见 `company/shared-services/ml-engineer-agent.md`
